...
function PhotoFill({ photo, radius = 0 }) {
  ...
  if (src) {
    return <img src={src} alt="" loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: radius, display: 'block' }} />;
  }
  return (
    <div style={{
...
// SCREEN: Welcome
// ═════════════════════════════════════════════
function WelcomeScreen({ t, onStart, onGallery, hasPhotos, guestCount, photoCount, guests }) {
    ...
    <div style={{ minHeight: '100%', position: 'relative', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 30px', textAlign: 'center', position: 'relative', zIndex: 2 }}>
        ...
        <div style={{ width: '100%', maxWidth: 320 }}>
          ...
          {hasPhotos && (
            <button onClick={onGallery} style={{ marginTop: 12, width: '100%', fontSize: 15, color: t.primaryColor, fontWeight: 700, fontFamily: "'Baloo 2', sans-serif", padding: 8 }}>
              See the live album →
            </button>
          )}
        </div>
      ...
      <div style={{ padding: '0 30px 26px', textAlign: 'center', position: 'relative', zIndex: 2 }}>
        <a href="host.html" style={{ fontSize: 13.5, color: '#9A93AC', fontWeight: 600, textDecoration: 'none' }}>
          Hosting a party? Create your own album →
        </a>
      </div>
    </div>
  );
            ...
            <div key={p.id} style={{
              ...
              <div style={{ padding: '12px 15px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <Avatar name={p.uploader} size={32} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12.5, color: '#A7A0B6' }}>{relTime(p.createdAt)}</div>
                  </div>
...
function App() {
  ...
  const toggleLike = (id) => {
    ...
    window.PartyPix.like(id, !next).catch(() => {});
  };
    ...
    <div style={{ position: 'relative' }}>
      <div className="phone-shell">
        <div style={{ position: 'relative', height: '100%', display: 'flex', flexDirection: 'column', background: screen === 'gallery' ? '#FBFAFE' : '#FFFDFB' }}>
          ...
          {toast && (
            <div style={{ position: 'absolute', left: 16, right: 16, bottom: 24, zIndex: 200,
              background: '#2A2140', color: '#fff', padding: '13px 16px', borderRadius: 14,
              fontSize: 14, fontWeight: 600, textAlign: 'center', boxShadow: '0 10px 30px rgba(0,0,0,0.3)',
              animation: 'popIn 0.3s ease both' }}>{toast}</div>
          )}
